...
st.title("🎬 AI Clip Editor + B-Roll Generator + SEO & Branding")
...
if st.selectbox("📂 Load Project", ["Select"] + list(st.session_state.saved_projects.keys())) != "Select":
        ...
        with st.spinner("Searching transcript with DOAC AI..."):
            ...
            if response.status_code != 200:
                ...
                if not results:
                    ...
                    ordered_clips = st.multiselect("🧩 Select & Order Clips for Final Timeline", [c[0] for c in clip_options], default=[c[0] for c in clip_options])
                    ...
                    if guest_name:
                        st.subheader("🌐 Guest Clip Sources")
